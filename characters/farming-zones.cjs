...
var partyFarmingZones = (() => {
  ...
  function resolve(catalog, ids, location) {
    if (!location) return null;
    if (location.shapes || location.allOf) return location;
    return zones(catalog, ids).find(
      (a) => a.map === location.map && (a.id === location.id || a.shapes && a.shapes.some(
        (s) => location.boundary && JSON.stringify(s.boundary) === JSON.stringify(location.boundary)
      ) || a.x === location.x && a.y === location.y || contains(a, location, 0, 1))
    ) || location;
  }
  ...
  return __toCommonJS(farming_zones_exports);
})();
if (typeof module === "object" && module.exports) module.exports = partyFarmingZones; else globalThis.partyFarmingZones = partyFarmingZones;
